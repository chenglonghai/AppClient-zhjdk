...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model.downLoadState == DownLoadStateDownloadfinished && tableView.editing != YES) {
            NSLog(@"---=====%@",[FRObjectToDictionary getObjectData:model]);
        NSLog(@"___%@", model.resumeDataPath);
        NSDirectoryEnumerator *enumerator = [[NSFileManager defaultManager] enumeratorAtPath:[SLFileManager getDownloadRootDir] ];
        
        NSLog(@"___XXX___%@",[SLFileManager getDownloadRootDir] );
        for (NSString *fileName in enumerator)
        {
            NSLog(@"__ZZZ___%@",fileName);
        }
